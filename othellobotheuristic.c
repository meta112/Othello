/*
Program to play against an Othello bot, written for APS105, a first-year C course in the UofT engineering program.
Leaderboard: http://aps105.ece.utoronto.ca:8090/result/zhuryan1/407922 
AI uses the minimax algorithm to search for optimal moves, with alpha-beta pruning to increase search efficiency.
Upon reaching a board position after (int depth) moves, it evaluates the position with a heuristic accounting for
number of pieces each player has, the number of legal moves each player currently has, and the number of corners
that each player possesses, linearly weighting each factor to assign the position a score.
Evaluation heuristic inspired by this paper, among other research: https://courses.cs.washington.edu/courses/cse573/04au/Project/mini1/RUSSIA/Final_Paper.pdf

This version typically searches 6 moves deep within a second (sometimes much faster), though it is not guaranteed.
...
In particular, I attempted to implement a stability factor, but I was unable to develop one that defeated the 
current heuristic when playing them against each other, so I removed it from this file (Its implementation is in 
a different version)

How to use:
Enter a single, even number between 4 and 26 inclusive. (e.g. entering 8 creates an 8x8 board)
Enter a single capital letter, B or W, to select the computer's colour (Black moves first in Othello)
Place your pieces by typing in 2 lowercase letters with no spaces in between - first the row, then the column.
e.g. entering ab will place a piece next to the top left corner - the first row and the second column.
*/
